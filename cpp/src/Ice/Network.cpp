...
string
IceInternal::SOCKSNetworkProxy::getHost() const
{
    if(!_host.empty())
    {
        return _host;
    ...
    {
        return inetAddrToString(_address);
    }
}

int
IceInternal::SOCKSNetworkProxy::getPort() const
{
    if(!_host.empty())
    {
        return _port;
...
std::string
IceInternal::fdToString(SOCKET fd, const NetworkProxyPtr& proxy, const std::string& host, int port)
{
    if(!proxy)
    {
        return fdToString(fd);
    }
    else
    {
        ostringstream s;
